...
- (void)processAsset
{
    ...
    {
            DLog(@"Error reading from file at URL: %@", self.url);
        return;
    }
...
- (void)processPlayerItem
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if (error)
        {
            DLog(@"DisplayLink created with error:%d", error);
            displayLink = NULL;
        }
...
- (BOOL)readNextVideoFrameFromOutput:(AVAssetReaderOutput *)readerVideoTrackOutput;
{
    if (reader.status == AVAssetReaderStatusReading && ! videoEncodingIsFinished)
    {
        ...
        if (sampleBufferRef) 
        {
            //DLog(@"read a video frame: %@", CFBridgingRelease(CMTimeCopyDescription(kCFAllocatorDefault, CMSampleBufferGetOutputPresentationTimeStamp(sampleBufferRef))));
...
- (BOOL)readNextAudioSampleFromOutput:(AVAssetReaderOutput *)readerAudioTrackOutput;
{
    if (reader.status == AVAssetReaderStatusReading && ! audioEncodingIsFinished)
    {
        ...
        {
            //DLog(@"read an audio frame: %@", CFBridgingRelease(CMTimeCopyDescription(kCFAllocatorDefault, CMSampleBufferGetOutputPresentationTimeStamp(audioSampleBufferRef))));
...
//
//            if (!texture || err) {
//                DLog(@"Movie CVOpenGLESTextureCacheCreateTextureFromImage failed (error: %d)", err);
...
//            outputTexture = 0;
    ...
    if (_runBenchmark)
    {
        ...
        DLog(@"Current frame time : %f ms", 1000.0 * currentFrameTime);
    }
}
