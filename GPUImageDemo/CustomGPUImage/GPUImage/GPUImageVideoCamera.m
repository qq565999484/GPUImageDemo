...
//    if (captureAsYUV && [GPUImageContext deviceSupportsRedTextures])
	...
	{
		DLog(@"Couldn't add video output");
        return nil;
	}
...
//            
//            if (!texture || err) {
//                DLog(@"Camera CVOpenGLESTextureCacheCreateTextureFromImage failed (error: %d)", err);
...
//            outputTexture = 0;
        ...
        if (_runBenchmark)
        {
            ...
            if (numberOfFramesCaptured > INITIALFRAMESTOIGNOREFORBENCHMARK)
            {
                ...
                DLog(@"Average frame time : %f ms", [self averageFrameDurationDuringCapture]);
